...
if os.path.exists("lstm_data.csv"):
    ...
    if (datetime.today() - timedelta(1)).date().strftime('%Y-%m-%d') == str(data.index.date[-1]):
        yesterday = datetime.today() - timedelta(61)
        yesterday = yesterday.strftime("%d/%m/%Y")
        get_load_data(yesterday)
        data = get_data()
        day_to_clip_from = datetime.today() - timedelta(121)
...
dt_df_last_nlags = scaler.transform(dt_df_last_nlags)  # df to numpy array
